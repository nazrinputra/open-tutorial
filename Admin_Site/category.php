...
<head>
	...
	<script type="text/javascript">
		...
		{
			var answer=confirm("Do you want to bar this category?");
			return answer;
		}
	</script>
</head>
					...
					<div id="tab-1" class="tab">
						<article>
							<div class="text-section">
								<h1>Administrator</h1>
								<p>View categories in the database.</p>
							</div>
							...
							<div class="text-section-black">
								...
								?>
									<tr>
										...
										<td><?php echo $row_category["Category_Desc"];?></td>
										<td><?php if($row_category["isDelete"]==0){echo 'Active';}else{echo 'Barred';}?></td>
										<td>
											<a href="edit_category.php?cid=<?php echo $row_category["Category_ID"]; ?>">
												<img src="images/update.png" width="30px" height="30px;"/>
											</a>
										</td>
										<?php
										if($row_category["isDelete"]==1)
										{
										?>
										<td>
											<a href="category.php?a_cid=<?php echo $row_category["Category_ID"]; ?>">
										...
										?>
										<td>
											<a href="category.php?cid=<?php echo $row_category["Category_ID"]; ?>">
								...
								<br><br><br>
								<a href="add_category.php"><input type="button" name="add_cat" id="add_cat" style="margin-left:250px;" value="Add Category"/></a>
							</div>
						</article>
					</div>
				</div>
...
<?php	
	...
	{
		$cid=$_REQUEST["cid"];
		mysql_query("update category set isDelete=1 where Category_ID = $cid");
	...
	if(isset($_REQUEST["a_cid"]))
	{
		...
		mysql_query("update category set isDelete=0 where Category_ID = $a_cid");
		header("Location: category.php");
	}
?>